...
### 11.3.6 SCC Runbook（来自SSOT）

**目标**: 把SCC的"任务执行与审计"固化为**可运行的状态机**

**核心原则**:
- **事实源优先**: 事实来自 `artifacts/` / `evidence/` / 数据库与可重放日志
- **Patch-only执行**: Executor只能提交最小diff；验证由Verifier/测试裁决
- **事件化交付**: 每次动作都要落盘事件（task/event/evidence）
- **失败可收敛**: 重试≤3；失败进入DLQ；必须给出下一步"可执行修复任务"
...
| 阶段 | 目标 | 产物 |
|------|------|------|
| **SEARCH** | 找到可验证的失败点/缺口 | 证据路径 + 最小复现步骤 |
| **HYPOTHESIS** | 提出1-3个可证伪假设 | 验证方法与预期输出 |
| **FREEZE** | 冻结要改的范围与可接受的副作用 | `acceptance_tests` + `rollback_plan` |
| **ACT** | 在冻结范围内最小改动 | patch + 执行日志 + 产物路径 |
| **VERIFY** | 只认测试/验证器输出 | exit_code、golden、回归用例 |

**重试/DLQ/升级策略**:
- **重试（≤3次）**: 每次必须修改假设/验证方式/冻结范围之一
- **DLQ条件**: 3次重试后仍失败 / 需要外部依赖 / 风控/登录/第三方不可控
- **DLQ落盘**: fail_code、阻塞条件、继续执行的最小触发条件
- **升级优先级**: 扩验证→扩执行范围→升模型→升权限
