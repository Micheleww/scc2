...
| 任务类型 | 上下文窗口 | 预算分配 | 特殊处理 |
|----------|-----------|----------|----------|
| **简单代码生成** | 8K tokens | 目标10% + 代码50% + 历史30% + 输出10% | 历史可完全省略 |
| **复杂功能实现** | 32K tokens | 目标15% + 相关文件45% + 架构25% + 输出15% | 保留关键设计文档 |
| **代码审查** | 16K tokens | PR描述10% + 变更文件60% + 规范20% + 输出10% | 聚焦变更范围 |
| **架构设计** | 64K tokens | 目标20% + 现有架构40% + 约束25% + 输出15% | 保留完整架构图 |
| **Bug修复** | 16K tokens | Bug描述15% + 相关代码50% + 历史20% + 输出15% | 保留错误日志 |
| **文档编写** | 16K tokens | 大纲15% + 参考文档45% + 示例25% + 输出15% | 保留格式模板 |
...
context_budget_rules:
  ...
  complexity_multiplier:
    simple: 0.5      # 简单任务使用50%预算
    medium: 1.0      # 中等任务使用100%预算
    complex: 1.5     # 复杂任务可使用150%预算（触发扩展）
  
  # 根据文件大小调整
  file_size_thresholds:
    small: 1000      # < 1KB 文件完整包含
    medium: 10000    # 1-10KB 文件摘要包含
    large: 50000     # > 50KB 文件必须排除或极度摘要
  
  # 紧急模式（当接近限制时）
  emergency_mode:
    trigger: "tokens > 0.9 * max_context"
    actions:
      - 压缩历史为单句摘要
      - 移除非关键pins
      - 使用更高层的map摘要
