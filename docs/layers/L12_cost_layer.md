...
```
1. 预算申请
   - 项目负责人提交预算申请（project_id + 预估token量 + 时间范围）
   - 必须包含预算理由和预期产出

2. 预算审核
   - factory_manager审核预算合理性
   - 检查历史成本数据和效率指标
...
4. 超预算处理
   - 达到80%: 发送告警通知
   - 达到95%: 自动降级到Free Tier模型
   - 达到100%: 暂停非关键任务，仅保留紧急修复
```
...
| 策略 | 说明 | 预期节省 |
|------|------|---------|
| **缓存优化** | 利用上下文缓存/稳定前缀减少重复输入 | 20-40% |
| **模型降级** | 简单任务使用Free Tier模型 | 50-80% |
| **Pins精准化** | 缩小pins范围减少上下文token | 10-30% |
| **批量执行** | 合并小任务减少系统开销 | 15-25% |
| **结果缓存** | 幂等查询结果缓存复用 | 变量 |
