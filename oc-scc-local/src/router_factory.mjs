function registerFactoryRoutes({ router }) {
  ...
  router.get("/factory/policy", async (ctx) => {
    ...
    if (!fp) return { type: "json", status: 404, body: { error: "missing_factory_policy", file: "factory_policy.json" } }
  ...
  router.get("/factory/routing", async (ctx) => {
    ...
    return { type: "json", status: 200, body: { event_type: et, lane, factory_policy: "factory_policy.json" } }
  })
}
