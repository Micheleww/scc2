...
def opencode_models_from_cache(cache: Dict[str, Any]) -> List[ModelRecord]:
    ...
    for provider_id, provider in cache.items():
        ...
        for m in items:
            ...
            # For OpenRouter entries within opencode, provider_id == "openrouter".
...
def fetch_openrouter_models(api_key: str, timeout_s: int = 30) -> List[Dict[str, Any]]:
...
def openrouter_models_from_api(models: List[Dict[str, Any]]) -> List[ModelRecord]:
    out: List[ModelRecord] = []
    for m in models:
        if not isinstance(m, dict):
            continue
        ...
        is_free = False
        if isinstance(prompt_p, (int, float)) and isinstance(completion_p, (int, float)) and prompt_p == 0 and completion_p == 0:
            is_free = True
        if ":free" in mid:
            is_free = True
        ...
        out.append(
            ModelRecord(
                ...
                context_length=ctx_i,
                cost=ModelCost(
                    input=float(prompt_p) if isinstance(prompt_p, (int, float)) else None,
                    output=float(completion_p) if isinstance(completion_p, (int, float)) else None,
                ),
                caps=ModelCaps(
                    toolcall=bool(m.get("supports_tools", False)) if "supports_tools" in m else False,
                    reasoning=False,
                    vision=bool(m.get("multimodal", False)) if "multimodal" in m else False,
                    temperature=True,
                ),
                is_free=bool(is_free),
                raw={"provider": m.get("provider"), "top_provider": m.get("top_provider")},
            )
        )
    return out
