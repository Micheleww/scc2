...
```
          ...
          ▼                            ▼                            ▼
┌──────────────────┐    ┌──────────────────┐    ┌──────────────────┐
│  L1 代码层        │    │  L3 文档层        │    │  L10 工作空间层   │
│  治理·API·宪法    │    │  SSOT·Docflow    │    │  workspace·输入  │
└────────┬─────────┘    └────────┬─────────┘    └────────┬─────────┘
         │                       │                       │
         ▼                       ▼                       ▼
┌──────────────────┐    ┌──────────────────┐    ┌──────────────────┐
│  L2 任务层        │───▶│  L4 提示词层      │◀───│  L5 模型层        │
│  契约·Task Bundle │    │  角色·技能·Pack   │    │  模型配置·升级    │
└────────┬─────────┘    └────────┬─────────┘    └──────────────────┘
         │                       │
         ▼                       ▼
┌──────────────────┐    ┌──────────────────┐    ┌──────────────────┐
│  L11 路由与调度层  │───▶│  L6 Agent层       │◀───│  L7 工具层        │
│  路由·调度·DLQ    │    │  执行·状态机      │    │  工具·技能守卫    │
└────────┬─────────┘    └────────┬─────────┘    └────────┬─────────┘
         │                       │                       │
         ▼                       ▼                       ▼
┌──────────────────┐    ┌──────────────────┐    ┌──────────────────┐
│  L12 成本与预算层  │    │  L8 证据与裁决层   │    │  L13 安全与权限层  │
│  Token·预算·告警  │    │  证据·裁决·fail   │    │  13道CI门·RBAC   │
└────────┬─────────┘    └────────┬─────────┘    └────────┬─────────┘
         │                       │                       │
         ▼                       ▼                       ▼
┌──────────────────┐    ┌──────────────────┐    ┌──────────────────┐
│  L14 质量与评测层  │    │  L15 变更与发布层  │    │  L9 状态与记忆层   │
│  质量门·缺陷分类  │    │  版本·发布·回滚    │    │  状态·记忆·上下文 │
└────────┬─────────┘    └────────┬─────────┘    └──────────────────┘
         │                       │
         └───────────┬───────────┘
                     ▼
...
**阅读顺序建议**:
- **新手**: L1(宪法) → L2(任务) → L6(执行) → L8(证据) → L17(术语)
- **Agent开发者**: L4(角色) → L6(执行) → L7(工具) → L13(安全)
- **运维/管理**: L11(调度) → L12(成本) → L16(观测) → L14(质量)
...
| **角色规范** | [L4提示词层 - RoleSpec](../../docs/layers/L4_prompt_layer.md) |
| **Capability Catalog** | [L4提示词层 - Capability Catalog](../../docs/layers/L4_prompt_layer.md) |
...
- **2026-02-08**: 增加17层交互关系图；去重SSOT附录；修复编号错误；统一引用方式
