---
oid: 01KGFSKMVGF0GQ0SGKNHH5ARV8
layer: DOCOPS
...
→ Strategy / Policy (Action Templates)
→ Execution
...
工程对应：
- docs/ssot/01_conventions/constitution/QUANTSYS_CONSTITUTION_V1.6.md（本宪法）
...
#### M9.1 Strategy Layer 的系统定位

##### 9.1.1 Strategy Layer 的定义
Strategy Layer 是一组“行为模板（Action Templates）”，
用于在已通过 belief、risk、execution 审批的前提下，
define 如何参与市场。

##### 9.1.2 Strategy Layer 不被视为优势来源
- 系统优势来自：
  - 信息提取（Factor）
  - 市场理解（Market Belief）
  - 风险约束（w / Risk Budget）
...
##### 9.1.3 空仓是合法且常态的策略结果
- Strategy 不保证持续持仓
- Strategy 不以“必须交易”为目标

#### M9.2 Strategy Layer 的权限边界（强制冻结）

##### 9.2.1 Strategy 明确禁止
- 直接使用价格、因子、概率计算仓位
- 修改 w、risk budget、per_unit_risk
- 绕过 Execution / Circuit Breaker
- 以内嵌函数方式承担风险管理职责

##### 9.2.2 Strategy 明确允许
- 定义入场与出场的结构条件
- 定义风险的“消耗方式”（一次性 / 持续）
- 定义持仓管理的动作逻辑（如加减仓规则，但不定义规模）
- 在 Execution 允许范围内选择执行路径

#### M9.3 Strategy 的输入与输出（制度化）

##### 9.3.1 Strategy 的合法输入
- 已校准的 Market Belief（只读）
- 当前 w（只读）
- 已批准的 risk budget（只读）
- 市场状态标签（如趋势 / 震荡 / 模糊）

##### 9.3.2 Strategy 的合法输出
- 行为请求（Action Intent），包括：
  - enter / exit / hold
  - entry_type（breakout / pullback / mean / etc）
  - exit_logic（time / price / condition）

##### 9.3.3 Strategy 不允许输出
- 仓位大小
- 杠杆倍数
- 风险预算调整请求

#### M9.4 Strategy 与其他模块的关系

##### 9.4.1 与 Market Belief
- Strategy 不评估概率
- Strategy 只消费 belief 给出的“环境许可”

##### 9.4.2 与 Risk / w
- Strategy 不修改风险
- Strategy 只能在“已允许风险”内行动

##### 9.4.3 与 Execution
- Strategy 发起行为请求
- Execution 有最终否决权

#### M9.5 Strategy 失败的制度性定义

##### 9.5.1 Strategy 失败 ≠ 系统失败
- Strategy 可以被暂停、替换或降级
- 不得触发系统级风险放大

##### 9.5.2 Strategy 失败形态必须可标注
- 多次小亏型
- 少数大亏型
- 执行失败型

用于风险与策略的事后评估，但不用于实时仓位调整。

#### M9.6 Strategy Layer 的工程对应
- strategies/trend.py
...
- 每次决策至少可追溯：
  ...
  - 是否因风险规则被限制或拒绝
  - 使用了哪个行为模板
  - 行为请求是否被 Execution 接受或拒绝
  - 最终是否产生持仓

验收：
- 给任意一笔交易能回答：当时的 w/状态/风控/成本门禁/信念健康度/仓位计算依据/策略行为请求 是什么

## 3️⃣ 模块关系说明

### 3.1 从 belief 到仓位的完整链路
```
Raw Belief → Calibration → Scored Belief → w (风险强度) → effective_risk (风险预算) → per_unit_risk (单位风险) → position_size (仓位) → Strategy (行为请求) → Execution
```
...
| Strategy | MarketBeliefCalibrated, w, effective_risk, position_size | Action Intent | 定义如何参与市场 | 与 position_size 同频 |
| Execution | Action Intent, position_size | Fills | 安全执行订单 | 与 Action Intent 同频 |

## 4️⃣ 与既有模块的关系说明
- 校准与评分属于 M4（Market Belief）的治理子层
- 仓位计算（M7）是新增模块，位于风险预算与策略之间
- Strategy Layer（M9）被明确定义为“行为模板”集合，仅负责定义参与方式
- 不修改 M2（Features）的职责
- 不替代 M5（State & w），只作为其下游扩展
- 不削弱断路器的最高优先级
- Execution（M10）正式纳入风险控制体系，作为风险系统的延伸
- Strategy 仅发起行为请求，Execution 拥有最终否决权

## 5️⃣ V1.5 → V1.6 变更摘要
1. 正式明确 Strategy Layer 的系统定位：行为模板集合，非优势来源
2. 冻结 Strategy Layer 的权限边界，明确禁止与允许的操作
3. 制度化 Strategy 的输入与输出，限定为行为请求，禁止输出仓位
4. 明确 Strategy 与其他模块的关系，强调其消费性角色
5. 定义 Strategy 失败的制度性标准，区分策略失败与系统失败
6. 强化 Strategy 层面的可观测性要求，确保每一次决策可追溯
7. 更新流水线描述，明确 Strategy 为“Action Templates”
8. 完善模块关系表，补充 Strategy 与 Execution 的交互
...
- Strategy Layer 必须严格作为“行为模板”存在，不得承担预测、概率或仓位计算职责

## 验收提示词
**TaskCode: QUANTSYS_CONSTITUTION_V1_6_UPDATE_001**

**提交**
- 最终回复以 SUBMIT 盖章
- ≤8 行总结：Strategy Layer 在 QuantSys 中的真正作用
