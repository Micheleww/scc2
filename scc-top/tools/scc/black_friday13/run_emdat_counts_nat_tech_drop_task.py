...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run EM-DAT robustness: Natural+Technological + drop + post2000.")
    ap.add_argument("--task-id", default="BLACK_FRIDAY13_EMDAT_COUNTS_NAT_TECH_DROP_V010")
    ap.add_argument(
        "--contract-ref",
        default="docs/ssot/04_contracts/black_friday13/contract_black_friday13_emdat_counts_nat_tech_drop_v010.json",
    )
    ap.add_argument("--area", default="control_plane")
    args = ap.parse_args()

    panel = _REPO_ROOT / "artifacts" / "black_friday13" / "ingest" / "panel__nat_tech__drop__post2000.parquet"
    if not panel.exists():
