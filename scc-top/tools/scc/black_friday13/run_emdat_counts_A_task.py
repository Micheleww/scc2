...
def main() -> int:
    ap = argparse.ArgumentParser(description="Create+run SCC contract task for EM-DAT count analysis (Variant A main display).")
    ap.add_argument("--task-id", default="BLACK_FRIDAY13_EMDAT_COUNTS_A_V010")
    ap.add_argument(
        "--contract-ref",
        default="docs/ssot/04_contracts/black_friday13/contract_black_friday13_emdat_counts_a_v010.json",
    )
    ap.add_argument("--area", default="control_plane")
    args = ap.parse_args()

    # Require ingest output exists.
    panel = _REPO_ROOT / "artifacts" / "black_friday13" / "ingest" / "panel__natural__drop__post2000.parquet"
    if not panel.exists():
