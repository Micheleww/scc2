...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run EM-DAT robustness: Natural + uniform_month + post2000.")
    ap.add_argument("--task-id", default="BLACK_FRIDAY13_EMDAT_COUNTS_NATURAL_UNIFORM_MONTH_V010")
    ap.add_argument(
        "--contract-ref",
        default="docs/ssot/04_contracts/black_friday13/contract_black_friday13_emdat_counts_natural_uniform_month_v010.json",
    )
    ap.add_argument("--area", default="control_plane")
    args = ap.parse_args()

    panel = _REPO_ROOT / "artifacts" / "black_friday13" / "ingest" / "panel__natural__uniform_month__post2000.parquet"
    if not panel.exists():
