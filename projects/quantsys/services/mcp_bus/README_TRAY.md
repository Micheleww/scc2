# MCP服务器托盘程序使用指南

## 🚀 快速开始

### 一键安装（推荐）

**以管理员身份运行PowerShell**，执行：

```powershell
cd d:\quantsys\tools\mcp_bus
.\install_all.ps1
```

或者双击运行：`快速安装.bat`

### 手动安装

1. **创建桌面快捷方式**
   ```powershell
   .\create_desktop_shortcut_tray.ps1
   ```

2. **设置开机自启动**（需要管理员权限）
   ```powershell
   .\setup_autostart_tray.ps1
   ```

3. **安装依赖**（如果未安装）
   ```powershell
   pip install pystray pillow
   ```

## 📋 功能特性

### ✅ 已实现功能

1. **开机自启动** ✓
   - 服务器随Windows启动自动运行
   - 使用Windows计划任务实现
   - 后台运行，无窗口

2. **桌面快捷方式** ✓
   - 保留桌面快捷方式
   - 双击即可启动
   - 自动隐藏窗口

3. **Windows系统托盘** ✓
   - 后台运行，不显示任务栏窗口
   - 可以关闭任务栏窗口，不影响服务器状态
   - 右键菜单快速访问功能

4. **状态颜色指示** ✓
   - 🟢 **绿色**：服务器正常运行，所有服务正常
   - 🟡 **黄色**：服务器运行但部分服务异常
   - 🔴 **红色**：服务器无法访问或严重错误
   - ⚪ **灰色**：服务器启动中或状态未知

## 🎨 托盘图标颜色说明

| 颜色 | 状态 | 说明 | 操作建议 |
|------|------|------|----------|
| 🟢 **绿色** | 正常运行 | 服务器正常运行，所有服务正常 | 无需操作 |
| 🟡 **黄色** | 部分异常 | 服务器运行但部分服务异常（Freqtrade未启动或OKX连接失败） | 检查服务状态 |
| 🔴 **红色** | 无法访问 | 服务器无法访问或严重错误 | 检查服务器是否运行 |
| ⚪ **灰色** | 启动中/未知 | 服务器启动中或状态未知 | 等待状态更新 |

## 📱 托盘菜单功能

右键点击托盘图标，可以访问：

- **状态**：显示当前服务器详细状态信息
- **打开仪表板**：在浏览器中打开主仪表板 (`http://127.0.0.1:18788/`)
- **打开FreqUI**：在浏览器中打开FreqUI界面 (`http://127.0.0.1:18788/frequi`)
- **打开Web查看器**：在浏览器中打开ATA消息查看器 (`http://127.0.0.1:18788/viewer`)
- **打开监控面板**：在浏览器中打开监控面板 (`http://127.0.0.1:18788/monitoring`)
- **退出**：停止服务器并退出托盘程序

## 🔧 使用方法

### 启动服务器

**方式1：双击桌面快捷方式**
- 双击 `MCP服务器（托盘图标）.lnk`
- 服务器在后台启动，不显示窗口
- 查看系统托盘图标确认状态

**方式2：使用PowerShell脚本**
```powershell
.\start_mcp_server_tray.ps1
```

**方式3：使用批处理文件**
```cmd
start_mcp_server_tray.bat
```

**方式4：直接运行Python脚本**
```powershell
python server_tray_enhanced.py
```

### 查看服务器状态

1. **查看托盘图标颜色**：右下角系统托盘
2. **右键点击托盘图标**：选择"状态"查看详细信息
3. **访问Web界面**：通过托盘菜单快速访问

### 停止服务器

右键点击托盘图标 → 选择"退出"

## ⚙️ 管理开机自启动

### 查看自启动任务

```powershell
Get-ScheduledTask -TaskName "MCP Bus Server (Tray)"
```

### 删除自启动任务

```powershell
Unregister-ScheduledTask -TaskName "MCP Bus Server (Tray)" -Confirm:$false
```

### 禁用自启动任务

```powershell
Disable-ScheduledTask -TaskName "MCP Bus Server (Tray)"
```

### 启用自启动任务

```powershell
Enable-ScheduledTask -TaskName "MCP Bus Server (Tray)"
```

### 手动运行自启动任务

```powershell
Start-ScheduledTask -TaskName "MCP Bus Server (Tray)"
```

## 🔍 状态检查机制

托盘程序每10秒自动检查一次服务器状态：

1. **健康检查**：访问 `/health` 端点
   - 失败 → 🔴 红色

2. **Freqtrade状态检查**：访问 `/api/freqtrade/status` 端点
   - 检查WebServer是否运行

3. **OKX状态检查**：访问 `/api/exchange/okx/status` 端点
   - 检查连接状态

4. **状态判断**：
   - 健康检查失败 → 🔴 红色
   - Freqtrade运行正常 → 🟢 绿色（OKX不是必需服务）
   - 服务器运行但Freqtrade未运行 → 🟡 黄色（如果启用了自动启动）
   - 无法获取状态 → ⚪ 灰色

## 🐛 故障排除

### 托盘图标不显示

1. **检查依赖是否安装**：
   ```powershell
   python -c "import pystray, PIL"
   ```

2. **如果未安装，执行**：
   ```powershell
   pip install pystray pillow
   ```

3. **检查Python版本**（需要Python 3.7+）

### 服务器无法启动

1. **检查端口是否被占用**：
   ```powershell
   netstat -ano | findstr :8000
   ```

2. **尝试手动启动查看错误**：
   ```powershell
   python server_tray_enhanced.py
   ```

3. **检查日志文件**（如果有）

### 开机不自启动

1. **确认已以管理员身份运行安装脚本**
2. **检查任务计划程序**：
   ```powershell
   Get-ScheduledTask -TaskName "MCP Bus Server (Tray)"
   ```
3. **检查任务是否启用**：
   ```powershell
   (Get-ScheduledTask -TaskName "MCP Bus Server (Tray)").State
   ```

### 状态颜色不正确

1. **检查服务器是否正常运行**：
   ```powershell
   curl http://127.0.0.1:18788/health
   ```

2. **检查服务状态**：
   ```powershell
   curl http://127.0.0.1:18788/api/freqtrade/status
   curl http://127.0.0.1:18788/api/exchange/okx/status
   ```

3. **等待10秒让状态检查更新**

### 关闭任务栏窗口后服务器停止

确保使用托盘版本启动：
- ✅ 使用 `server_tray_enhanced.py`
- ✅ 使用 `start_mcp_server_tray.ps1`
- ✅ 使用桌面快捷方式
- ❌ 不要直接运行 `start_mcp_server.bat`（会显示窗口）

## 📁 文件说明

| 文件 | 说明 |
|------|------|
| `server_tray_enhanced.py` | 增强版托盘程序（带状态监控和颜色变化） |
| `server_tray.py` | 基础版托盘程序（无状态监控） |
| `setup_autostart_tray.ps1` | 设置开机自启动脚本 |
| `create_desktop_shortcut_tray.ps1` | 创建桌面快捷方式脚本 |
| `start_mcp_server_tray.ps1` | 启动托盘版本服务器（PowerShell） |
| `start_mcp_server_tray.bat` | 启动托盘版本服务器（批处理） |
| `install_all.ps1` | 一键安装脚本 |
| `快速安装.bat` | 一键安装脚本（批处理版本） |
| `TRAY_SETUP_GUIDE.md` | 详细设置指南 |

## ⚠️ 注意事项

1. **管理员权限**：设置开机自启动需要管理员权限
2. **依赖安装**：首次使用需要安装 `pystray` 和 `pillow`
3. **端口占用**：确保8000端口未被占用
4. **防火墙**：如果使用防火墙，确保允许Python访问网络
5. **状态更新**：状态检查每10秒更新一次，颜色变化可能有延迟
6. **关闭窗口**：使用托盘版本启动后，可以安全关闭任务栏窗口，服务器继续运行

## 🎯 最佳实践

1. **首次安装**：运行 `install_all.ps1` 完成所有设置
2. **日常使用**：双击桌面快捷方式启动
3. **状态监控**：通过托盘图标颜色快速判断服务器状态
4. **问题排查**：右键托盘图标 → "状态" 查看详细信息

## 📞 技术支持

如果遇到问题：

1. 查看 `TRAY_SETUP_GUIDE.md` 获取详细说明
2. 检查服务器日志
3. 查看托盘图标状态信息
4. 检查Windows任务计划程序
