...
from pending_order_tracker import OrderStatus, PendingOrderTracker
...
def test_order_window_tracker():
    ...
    # 测试2: 累计接近限制（9个订单，累计900，再加100会达到1000，应该被拒绝）
    for i in range(8):
        allowed, total = tracker.add_order(100.0, "BTC-USDT", "buy")
        print(f"订单{i + 2}: 金额=100 USDT, 允许={allowed}, 累计={total:.2f} USDT")
        assert allowed, f"订单{i + 2}应该被允许"

    # 测试3: 超过限制（第10个订单会使累计达到1000，应该被拒绝）
    allowed, total = tracker.add_order(100.0, "BTC-USDT", "buy")
    print(f"订单10: 金额=100 USDT, 允许={allowed}, 累计={total:.2f} USDT")
    assert not allowed, "订单10应该被拒绝（累计达到限制）"
