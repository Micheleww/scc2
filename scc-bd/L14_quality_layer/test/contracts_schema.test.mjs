...
function repoRootFromHere() {
  ...
  // L14_quality_layer/test -> repo root
  return path.resolve(__dirname, "..", "..")
}

function contractsDirFromHere() {
  const root = repoRootFromHere()
  // Contracts are in L2_task_layer/contracts
  return path.join(root, "L2_task_layer", "contracts")
}
...
test("contracts schemas enforce minimal strictness", () => {
  const contractsDir = contractsDirFromHere()
  const envelopePath = path.join(contractsDir, "envelope", "envelope.schema.json")
  const eventPath = path.join(contractsDir, "event", "event.schema.json")
  const submitPath = path.join(contractsDir, "submit", "submit.schema.json")
  const verdictPath = path.join(contractsDir, "verdict", "verdict.schema.json")
...
test("G6 schema tightening constraints are enforced", () => {
  const contractsDir = contractsDirFromHere()
  const childTaskPath = path.join(contractsDir, "child_task", "child_task.schema.json")
  const factoryPolicyPath = path.join(contractsDir, "factory_policy", "factory_policy.schema.json")
...
test("all contract schemas are valid JSON and have required fields", () => {
  const contractsDir = contractsDirFromHere()
  ...
  for (const p of schemaFiles) {
    ...
    assert.ok(schema.$schema, `schema should have $schema: ${rel}`)
    assert.ok(schema.type || schema.properties || schema.oneOf || schema.anyOf || schema.allOf || schema.$ref, 
              `schema should have type or properties or composition keywords: ${rel}`)
  }
})
