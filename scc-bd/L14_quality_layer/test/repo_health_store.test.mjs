...
import { loadRepoHealthState, repoUnhealthyActive, saveRepoHealthState } from "../../L1_code_layer/gateway/lib/repo_health_store.mjs"
...
test("repo_health_store load/save + repoUnhealthyActive", () => {
  ...
  assert.equal(repoUnhealthyActive({ state: st, nowMs: Date.now() }), false)
  ...
  assert.equal(repoUnhealthyActive({ state: st2, nowMs: Date.now() }), true)
})
