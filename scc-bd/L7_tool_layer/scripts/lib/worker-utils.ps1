...
function Sanitize-ForJson([string]$s) {
    if ($null -eq $s) { return "" }
    $t = [string]$s
    # JSON does not allow raw control chars 0x00-0x1F except tab, newline, carriage return
    return ($t -replace "[\u0000-\u0008\u000B\u000C\u000E-\u001F]", "")
}
...
function Test-ContextPackV1Required($job, [string]$base, [string]$workerId, [bool]$requireContextPackV1) {
    ...
    # board_split tasks don't require context pack v1
    $taskType = $null
    if ($job.PSObject.Properties.Name -contains "taskType") {
        $taskType = [string]$job.taskType
    }
    if ($taskType -eq "board_split") { return @{ valid = $true } }
