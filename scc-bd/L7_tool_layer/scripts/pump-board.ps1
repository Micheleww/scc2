...
while ($true) {
  try {
    ...
    if ($board2.counts) {
      $c = $board2.counts
      Write-Host ("[{0}] board total={1} parent={2} atomic={3} in_progress={4} ready={5} backlog={6}" -f (Get-Date -Format "HH:mm:ss"), $c.total, $c.parent, $c.atomic, ($c.byStatus.in_progress ?? 0), ($c.byStatus.ready ?? 0), ($c.byStatus.backlog ?? 0))
    }
  } catch {
